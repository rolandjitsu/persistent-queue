...
/// How hard a write is pushed toward disk before it counts as done.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum Durability {
    /// fsync every push and ack. Strongest, slowest.
    #[default]
    Sync,
    /// Do not fsync; rely on the OS page cache. Survives a process crash but not
    /// a power loss.
    None,
}
...
impl<S: Store> Producer<S> {
    ...
    // Commit the entry outside the lock; on failure, give back the reserved slot.
    fn write(&self, seq: u64, value: &[u8]) -> Result<(), S::Error> {
        let key = entry_key(seq);
        match self
            .shared
            .store
            .commit(&[Op::Put(&key, value)], self.shared.durable)
        {
            Ok(()) => Ok(()),
            Err(e) => {
                {
